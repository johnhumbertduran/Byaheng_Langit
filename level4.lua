...
lvl4Sound = audio.loadSound("sound/lvl4.wav")
--------------------------------------------
...
function onReplayBtnRelease(event)
	...
	storyboard.purgeScene( "rep4" )
	storyboard.gotoScene (  "rep4", "fade", 500 )
...
function scene:createScene( event )
	...
	bg1 = display.newImageRect( "im/lvl4_bg.jpg", 480, 320 )
	bg1.x = 240
	bg1.y = 160
	
	bg2 = display.newImageRect( "im/lvl4_bg.jpg", 480, 320 )
	bg2.x = bg1.width + 240
	bg2.y = 160
	...
	grcl1 = display.newImageRect( "im/ground_cloud4.png", 480, 60 )
	grcl1.x = 240
	grcl1.y = 290
	physics.addBody( grcl1, "static", { friction=100 } )

	grcl2 = display.newImageRect( "im/ground_cloud4.png", 480, 60 )
	...
	local playoptions =
{
	...
	local sheet = graphics.newImageSheet("im/character4.png", playoptions )
	...
	local holesheet = graphics.newImageSheet("im/monster_4.png", { width = 53, height =123, numFrames = 3} )
	...
	local holeShape = { -15,-60, 15,-60, 20,31, -20,31 }
	physics.addBody( hole, "static", { density=1.2, friction=0, bounce=0, shape=holeShape } )

	-- hole:scale(.5,.5)
	hole:play()

	gate1 = display.newImageRect("im/gate4.png", 198, 221)
	gate1.x = 25000
...
function scene:enterScene( event )
	...
	if pauseSound.isVisible == true then
		audio.play( lvl4Sound, { channel=1, loops=-1 } )
	end

	ueanGroup = display.newGroup()
	...
	function ubraCoin()
		...
		coinRanX = math.random(2000,3000)
	...
	function ubraUean()
		local uean = display.newImageRect("im/rain.png", 10, 10)
		uean.y = -10
		table.insert( ueanTable, uean )
		ueanGroup:insert(uean)

		local uean2 = display.newImageRect("im/rain.png", 10, 10)
		uean2.y = -10
		table.insert( ueanTable, uean2 )
		ueanGroup:insert(uean2)

		local uean3 = display.newImageRect("im/rain.png", 10, 10)
		uean3.y = -10
		table.insert( ueanTable, uean3 )
		ueanGroup:insert(uean3)

		local countRain = math.random( 3 )
		ueanRanX = math.random(0,500)
		ueanTimeRand = math.random(1000,2500)

		if ( countRain == 1 ) then
		
			uean.x = ueanRanX
			transition.to( uean, {x=-500, y=500, time=ueanTimeRand,
			onComplete = function() display.remove( uean ) end
		} )
			uean.alpha = 1
			
		elseif ( countRain == 2 ) then
			
			uean2.x = 600
			uean2.y = ueanRanX
			transition.to( uean2, {x=-200, y=500, time=ueanTimeRand,
			onComplete = function() display.remove( uean2 ) end
		} )
			uean2.alpha = 1
	
		elseif ( countRain == 3 ) then
			
			uean3.x = 490
			uean3.y = ueanRanX
			transition.to( uean3, {x=-1000, y=500, time=ueanTimeRand,
			onComplete = function() display.remove( uean3 ) end
		} )
			uean3.alpha = 1
	...
	-- ueantimer = timer.performWithDelay( 1, umueanKa, 0 )

	Runtime:addEventListener("enterFrame", ubraUean)

	lightning = display.newRect(240, 160, 480, 320)
	lightning:setFillColor(255)
	lightning.alpha = 0

	darkness = display.newRect(240, 160, 480, 320)
	...
	function thunder()
		...
		function darken2()
		dark2 = transition.to( darkness, {alpha = 0, time=2000, onComplete=darken2})
		end

		function darken1()
		dark1 = transition.to( darkness, {alpha = 1, time=0, onComplete=darken2})
		end

		function flashing4()
		flash4 = transition.to( lightning, {alpha = 0, time=1000, onComplete=darken1})
		end

		function flashing3()
		flash3 = transition.to( lightning, {alpha = 1, time=0, delay=1000, onComplete=flashing4})
		end

		function flashing2()
		flash2 = transition.to( lightning, {alpha = 0, time=250, onComplete=flashing3})
		end

		flash1 = transition.to( lightning, {alpha = 1, time=0, onComplete=flashing2})

	end

	thunderTimer = timer.performWithDelay( 2000, thunder, 0 )
					...
					if(currentLevel <= 4) then
						...
						-- function NextButton()
							...
							if (score > highScore[4]) then
								if (score >= 1600 and score < 2000) then
								...
								elseif (score >= 2000 and score < 2400) then
									function star1ReturnScale()
										star1Visible2 = transition.to( star1, { transition=easing.inExpo, xScale=1, yScale=1} )
									end
								star1Visible = transition.to( star1, { transition=easing.inExpo, xScale=3, yScale=3, alpha=1, onComplete=star1ReturnScale} )
...
function scene:exitScene( event )
	...
	physics.stop()
	audio.stop(1)
	Runtime:removeEventListener("enterFrame", ubraUean)
