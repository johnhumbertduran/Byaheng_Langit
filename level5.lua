...
lvl5Sound = audio.loadSound("sound/lvl5.wav")
--------------------------------------------
...
function onReplayBtnRelease(event)
	...
	storyboard.purgeScene( "rep5" )
	storyboard.gotoScene (  "rep5", "fade", 500 )
...
function scene:createScene( event )
	...
	bg1 = display.newImageRect( "im/lvl5_bg.jpg", 480, 320 )
	bg1.x = 240
	bg1.y = 160
	
	bg2 = display.newImageRect( "im/lvl5_bg.jpg", 480, 320 )
	bg2.x = bg1.width + 240
	bg2.y = 160
	...
	grcl1 = display.newImageRect( "im/ground_cloud5.png", 480, 60 )
	grcl1.x = 240
	grcl1.y = 290
	physics.addBody( grcl1, "static", { friction=100 } )

	grcl2 = display.newImageRect( "im/ground_cloud5.png", 480, 60 )
	...
	local playoptions =
{
	...
	local sheet = graphics.newImageSheet("im/character5.png", playoptions )
	...
	local holesheet = graphics.newImageSheet("im/bird.png", { width = 53, height =123, numFrames = 3} )
	...
	local holeShape = { 5,-38, 13,-9, 13,10, 10,36, -10,36, -13,10, -13,-9, -5,-38 }
	physics.addBody( hole, "static", { density=1.2, friction=0, bounce=0, shape=holeShape } )
	hole:rotate(-90)
	-- hole:scale(.5,.5)
	hole:play()

	gate1 = display.newImageRect("im/gate5.png", 198, 221)
	gate1.x = 30000
...
function scene:enterScene( event )
	...
	if pauseSound.isVisible == true then
		audio.play( lvl5Sound, { channel=1, loops=-1 } )
	end
				...
				function checkNextButton()
					...
					if(currentLevel <= 5) then
						...
						-- function NextButton()
							...
							if (score > highScore[5]) then
								if (score >= 2000 and score < 2400) then
								...
								elseif (score >= 2400 and score < 2800) then
									function star1ReturnScale()
										star1Visible2 = transition.to( star1, { transition=easing.inExpo, xScale=1, yScale=1} )
									end
								star1Visible = transition.to( star1, { transition=easing.inExpo, xScale=3, yScale=3, alpha=1, onComplete=star1ReturnScale} )
